...
def notify(users, label, project=None, extra_context={}, on_site=True, sender=None):
    ...
    for user in users:
        notice = Notice.objects.create(recipient=user, project=project, on_site=on_site,
            notice_type=NoticeType.objects.get(label=notice.label), sender=sender)
        if project and project.has_perm(user, OI_READ):
            if get_notification_setting(user, notice_type, project).should_send() and user.email and user.is_active: # Email
                notice.send(user, label, context)
...
class NoticeSetting(models.Model):
    ...
    send_every = models.IntegerField(default=0)
    project = models.ForeignKey(Project, null=True)
    
    def should_send():
        return self.last_notice + self.send_every > datetime.datetime.now()
...
def get_notification_setting(user, notice_type, project=None):
    setting, created = NoticeSetting.objects.get_or_create(user=user, notice_type=notice_type, project=project)
    return setting
...
class Notice(models.Model):
    ...
    def send(self):
        ...
        try:
            ...
            activate(language) # activate the user's language
        ...
        context = Context({"recipient": user, "sender": sender, "notice": ugettext(notice.notice_type.display),
        ...
        messages = get_formatted_messages(formats, notice.label, context)
