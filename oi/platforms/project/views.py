...
def savespec(request, id, specid='0'):
    ...
    #        filename = normalize("NFC", filename) #this encoding should work with next version of xsendfile
            ...
            if spec.file:
                spec.file.delete()
            path = ("%s%s_%s_%s"%(settings.TEMP_DIR,request.user.id,request_dict["ts"],filename))
...
#    filename = normalize("NFC", uploadedfile.name).encode('utf-8').
    ...
    tempfile = open("%s%s_%s_%s"%(settings.TEMP_DIR,request.user.id,ts,filename), 'wb+')
...
def deltmpfile(request, id):
    ...
    request_dict = QueryDict(request.body)
    if request.method == "POST":
        path = "%s%s_%s_%s"%(settings.TEMP_DIR,request.user.id,request_dict["ts"],request_dict["filename"])
        os.remove(path)
...
#    response['X-Sendfile'] = "%sproject/%s/%s"%(settings.MEDIA_ROOT,id,quote(filename.encode('utf8')))
    response['X-Sendfile'] = "%sproject/%s/%s"%(settings.MEDIA_ROOT,id,filename.encode('utf8'))
    try:
        response['Content-Length'] = os.path.getsize("%sproject/%s/%s"%(settings.MEDIA_ROOT,id,filename))
    except OSError:
        raise Http404
    return response
